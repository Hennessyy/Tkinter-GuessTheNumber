...
def check_answer():
    ...
    # Getting users input and attempting to convert it to integer data type.
    guess = entry_window.get()

    try:
        guess = int(guess)
    except ValueError:
        text.set("Your must enter a whole number between 1 and 99!")
    ...
    if 1 <= guess <= 99:
        ...
        text.set("Your number must be between 1 and 99")
        entry_window.delete(0, END)
    return
